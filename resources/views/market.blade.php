...
@section('maincontent')
<div class="bggallery pt-3">
<div class="container g-0 g-md-auto">
	<div class="row row-custom-margin">
		<!--<div class="col-12">
			<h1 class="text-md-start text-center">Товары</h1>
		</div>-->
...
<div class="modal modal-xl fade" id="update_post" tabindex="-1" aria-labelledby="update_post_label" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
       ...
       <form class="needs-validation-update" novalidate @submit.prevent="UpdatePost('market')">
		 <h2>Upload Post</h2>
  ...
  <div class="form-check form-switch ms-5">
    ...
    <input @change="imagechanged_product(i - 1)" :id="'file_' + (i - 1)" class="upload-input-img" type="file" accept="image/*" />
      ...
      <div class="modal-footer">
        ...
        <form class="needs-validation" novalidate @submit.prevent="UploadNewPost('market')">
		 <h2>Upload Post</h2>
		 <div class="row mt-5 mb-3">
		 	<div class="col-lg-4">
		 		<div class="col-12">
  ...
  <div class="form-check form-switch ms-5">
    <input
      ...
      :false-value="0"
    />
    <label class="form-check-label" for="statusSwitch5">
      В наличии
    </label>
  </div>
</div>
...
<div v-if="page_is_loading" class="row mt-0 row-custom-margin">
	<?php for ($i = 0; $i < 4; $i++): ?>
		<div class="col-10 col-md-4 col-lg-3 mt-3 mx-auto mx-md-0">
			<div class="w-100 feature-item position-relative bg-primary-features shadow-primary text-center p-3 card-primary">
				<div class="skeleton-loader product-item-size-img-skeleton"></div>
				<div class="card-body">
					<h5 class="card-title mt-3"><div class="skeleton-loader"></div></h5>
...
<div class="row mt-0 row-custom-margin">
    ...
    >
		...
		<div class="feature-item position-relative bg-primary-features shadow-primary text-center p-3 card-primary my-3" style="width:100%;">
			<div class="py-1 px-2 border-card">
			@auth
  ...
  @endauth
  @verbatim
		  <img :src="item.main_img" class="img-fluid img-card mb-3" style="object-fit: cover;">
		  <div class="card-body">
		    <h5 class="text-white mb-0 main-text mb-4">{{item.title}}</h5>
		    <p class="card-text row" v-html="item.description"></p>

		    <a
  data-bs-toggle="offcanvas"
  data-bs-target="#offcanvasTop"
  aria-controls="offcanvasTop"
  class="btn btn-dark py-2 px-4 mb-2"
  :class="{ disabled: item.status_sold == 0 }"
  :style="{ pointerEvents: item.status_sold == 0 ? 'none' : 'auto', opacity: item.status_sold == 0 ? 0.6 : 1 }"
>
  Заказать
</a>
		     <p v-if="item.status_sold == 1" class="display-4 text-primary text-card-type">В наличии</p>
  			<p v-else class="display-4 text-primary text-card-type">Нет в наличии</p>
